...
assign m0_awready = s_awready && (grant == 2'b00) && aw_phase_active;
assign m1_awready = s_awready && (grant == 2'b01) && aw_phase_active;
assign m2_awready = s_awready && (grant == 2'b10) && aw_phase_active;

always @(*) begin
    case (grant)
        2'b00: begin
            s_awid    = {2'b00, m0_awid};
            s_awaddr  = m0_awaddr;
            s_awvalid = m0_awvalid && aw_phase_active;   // ← fix
            s_awlen   = m0_awlen;
            s_awsize  = m0_awsize;
            s_awburst = m0_awburst;
        end
        2'b01: begin
            s_awid    = {2'b01, m1_awid};
            s_awaddr  = m1_awaddr;
            s_awvalid = m1_awvalid && aw_phase_active;   // ← fix
            s_awlen   = m1_awlen;
            s_awsize  = m1_awsize;
            s_awburst = m1_awburst;
        end
        2'b10: begin
            s_awid    = {2'b10, m2_awid};
            s_awaddr  = m2_awaddr;
            s_awvalid = m2_awvalid && aw_phase_active;   // ← fix
            s_awlen   = m2_awlen;
            s_awsize  = m2_awsize;
